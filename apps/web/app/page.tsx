...
const roleForTone: Record<string, UserRole> = { student: "student", teacher: "teacher", admin: "admin" };
  ...
  const roles = [
    {
      ...
      desc: "围绕项目迭代、文件上传、智能陪练与竞赛准备，把学、做、改组织成一个连续而清晰的成长界面。",
      points: ["项目推进与智能问答", "文档上传与即时诊断", "连续迭代中的成长反馈"],
      icon: "◉", tone: "student", cta: "进入学生端",
    ...
    <main className="home-page">
        ...
        <div className="home-nav-actions">
          {loggedRole ? (
            <>
              ...
              <button type="button" className="home-nav-btn home-nav-btn-ghost" onClick={() => { localStorage.removeItem("va_user"); setLoggedRole(null); }}>
                退出
              </button>
            </>
          ) : (
            <Link href="/auth/login" className="home-nav-btn home-nav-btn-primary">
              登录 / 注册
            </Link>
          )}
        </div>
      ...
      <section className="home-hero fade-up">
        <div className="home-hero-copy">
          ...
          <div className="home-hero-actions">
            <Link href="/auth/login" className="home-hero-action home-hero-action-primary">
              开始使用
              <svg width="18" height="18" viewBox="0 0 16 16" fill="none" style={{marginLeft:6}}><path d="M3 8h10M9 4l4 4-4 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
            </Link>
          </div>
      ...
      <section className="home-signal-band fade-up" aria-label="platform signals">
        <div className="home-signal-row home-signal-row-a">
          {[...signals, ...signals].map((item, index) => (
            <span key={`a-${index}`} className="home-signal-chip">{item}</span>
          ))}
        </div>
        <div className="home-signal-row home-signal-row-b">
          {[...signals.slice().reverse(), ...signals.slice().reverse()].map((item, index) => (
            <span key={`b-${index}`} className="home-signal-chip home-signal-chip-soft">{item}</span>
          ))}
        </div>
      </section>

      {/* ── Role Grid (cards link to /auth/login, auth guard does the rest) ── */}
      <section id="home-roles" className="home-role-grid">
        ...
        {roles.map((role, index) => {
          const thisRole = roleForTone[role.tone] ?? null;
          const dimmed = loggedRole !== null && thisRole !== loggedRole;
          ...
          return (
            <article
              key={role.title}
              className={`home-role-card home-role-card-${role.tone} fade-up${dimmed ? " home-role-card-dimmed" : ""}`}
