...
INTENTS: dict[str, dict] = {
    ...
    "funding_investment": {
        "keywords": ["融资", "投资", "天使轮", "A轮", "B轮", "VC", "估值",
                      "股权", "FA", "尽调", "term sheet", "BP", "投资人",
                      "风投", "种子轮", "pre-A", "融钱", "拿投资", "找投资",
                      "怎么融资", "多少钱", "融多少", "稀释", "对赌"],
        "desc": "学生想了解融资、估值、股权、投资人等创业融资实操",
        "agents": ["coach"],
        "need_web": True, "web_results": 3,
        "focused": True,
    },
    "company_operations": {
        "keywords": ["开公司", "注册公司", "营业执照", "工商注册", "法务",
                      "股权架构", "合伙协议", "合伙人", "股权分配", "税务",
                      "财务", "合规", "知识产权", "专利", "商标", "法律",
                      "公司注册", "有限责任", "合伙企业", "章程", "社保",
                      "代理记账", "发票", "纳税"],
        "desc": "学生想了解注册公司、法务、股权架构、财税合规等创业运营实操",
        "agents": ["tutor"],
        "need_web": True, "web_results": 3,
        "focused": True,
    },
    "startup_execution": {
        "keywords": ["招人", "团队管理", "现金流", "运营", "增长", "扩张",
                      "上市", "IPO", "并购", "获客", "留存", "转化率",
                      "增长黑客", "冷启动", "种子用户", "推广", "裂变",
                      "拉新", "促活", "复购", "私域", "社群运营",
                      "供应链", "外包", "招聘", "企业文化"],
        "desc": "学生想了解团队搭建、增长获客、运营管理、上市扩张等创业执行实操",
        "agents": ["coach"],
        "need_web": True, "web_results": 2,
        "focused": True,
    },
    ...
    if _llm.enabled:
        ...
        llm_r = _llm.chat_json(
            system_prompt=(
                ...
                "9. **funding_investment**: 学生在问融资、估值、股权、BP、投资人、天使轮/A轮等创业融资问题。\n"
                "10. **company_operations**: 学生在问注册公司、股权架构、合伙协议、法务、税务、知识产权等公司运营问题。\n"
                "11. **startup_execution**: 学生在问团队搭建、增长获客、冷启动、运营管理、现金流、上市等创业执行问题。\n"
...
def gather_context_node(state: WorkflowState) -> dict:
    ...
    comp_type = state.get("competition_type", "") or ""
    ...
    # ────────────────────────────────────────────────────────────────
    # STATIC + CONDITIONAL: Parallel I/O tasks (full weight)
    # ────────────────────────────────────────────────────────────────
    ...
    # Hypergraph teaching: conditional on file or diagnostic/entrepreneurship intents
    if is_file or intent in ("project_diagnosis", "evidence_check", "competition_prep", "pressure_test", "business_model",
                              "funding_investment", "startup_execution"):
...
def _fmt_hyper_for_analyst(hs: dict, hi: dict) -> str:
    """Analyst needs: risk warnings, critical missing dims, consistency issues, teaching risk edges."""
    parts = []
    if hs and hs.get("ok"):
        warnings = hs.get("pattern_warnings", [])
        for w in warnings[:3]:
            parts.append(f"超图风险预警: {w.get('warning','')}")
        missing = hs.get("missing_dimensions", [])
        critical = [m for m in missing if m.get("importance") in ("极高", "高")]
        if critical:
            parts.append("关键缺失维度: " + "; ".join(f"{m['dimension']}—{m.get('recommendation','')}" for m in critical[:3]))
        issues = hs.get("consistency_issues", [])
        for issue in issues[:2]:
            parts.append(f"一致性冲突: {issue.get('message','')}")
    edges = (hi or {}).get("edges", []) if isinstance(hi, dict) else []
    risk_edges = [e for e in edges if isinstance(e, dict) and "risk" in str(e.get("type", "")).lower()]
    if risk_edges:
        parts.append("教学超边(风险): " + "; ".join(
            str(e.get("teaching_note", "") or e.get("summary", ""))[:60] for e in risk_edges[:2]
        ))
    elif edges:
        parts.append("教学超边: " + "; ".join(
            f"{str(e.get('type',''))}:{str(e.get('teaching_note','') or e.get('summary',''))[:50]}"
            for e in edges[:2] if isinstance(e, dict)
        ))
    if hi and isinstance(hi, dict) and hi.get("summary"):
        parts.append(f"超图总结: {str(hi['summary'])[:120]}")
    return "\n".join(parts)


def _fmt_hyper_for_advisor(hs: dict, hi: dict) -> str:
    """Advisor needs: coverage score, incomplete loops, competitive/strategy edges."""
    parts = []
    if hs and hs.get("ok"):
        cov = hs.get("coverage_score", 0)
        parts.append(f"超图覆盖度: {cov}/10")
        tm = hs.get("template_matches", [])
        incomplete = [t for t in tm if t.get("status") != "complete"]
        for t in incomplete[:2]:
            parts.append(f"未闭合逻辑环: {t.get('name','')} 缺 {', '.join(t.get('missing_dimensions',[])[:3])}")
        missing = hs.get("missing_dimensions", [])
        if missing:
            parts.append("缺失维度: " + "; ".join(f"{m['dimension']}({m['importance']})" for m in missing[:4]))
    edges = (hi or {}).get("edges", []) if isinstance(hi, dict) else []
    comp_edges = [e for e in edges if isinstance(e, dict) and any(
        kw in str(e.get("type", "")).lower() for kw in ("competitive", "strategy", "value")
    )]
    target = comp_edges[:2] if comp_edges else edges[:2]
    if target:
        parts.append("教学超边(策略): " + "; ".join(
            f"{str(e.get('type',''))}:{str(e.get('teaching_note','') or e.get('summary',''))[:50]}"
            for e in target if isinstance(e, dict)
        ))
    return "\n".join(parts)


def _fmt_hyper_for_grader(hs: dict) -> str:
    """Grader needs: coverage score, critical missing for scoring impact."""
    if not hs or not hs.get("ok"):
        return ""
    parts = []
    cov = hs.get("coverage_score", 0)
    total = hs.get("total_dimensions", 10)
    covered = hs.get("covered_count", 0)
    parts.append(f"超图维度覆盖: {covered}/{total} (得分{cov}/10)")
    missing = hs.get("missing_dimensions", [])
    critical = [m for m in missing if m.get("importance") in ("极高", "高")]
    if critical:
        parts.append("评分受影响的缺失维度: " + "; ".join(f"{m['dimension']}({m['importance']})" for m in critical[:4]))
    return "\n".join(parts)


def _fmt_hyper_for_planner(hs: dict) -> str:
    """Planner needs: missing dims with recommendations, incomplete loops for task generation."""
    if not hs or not hs.get("ok"):
        return ""
    parts = []
    missing = hs.get("missing_dimensions", [])
    if missing:
        for m in missing[:4]:
            ...
            parts.append(f"待补维度: {m['dimension']}({m['importance']}) → {rec}" if rec else f"待补维度: {m['dimension']}({m['importance']})")
    tm = hs.get("template_matches", [])
    incomplete = [t for t in tm if t.get("status") != "complete"]
    for t in incomplete[:2]:
        parts.append(f"未完成闭环: {t.get('name','')} 缺少 {', '.join(t.get('missing_dimensions',[])[:3])}")
    return "\n".join(parts)


def _fmt_hyper_for_tutor(hs: dict, hi: dict) -> str:
    """Tutor needs: missing concepts for teaching, teaching edges for explanation."""
    parts = []
    if hs and hs.get("ok"):
        missing = hs.get("missing_dimensions", [])
        if missing:
            parts.append("学生尚未涉及的概念维度: " + ", ".join(m['dimension'] for m in missing[:5]))
        hubs = hs.get("hub_entities", [])
        if hubs:
            parts.append("学生项目核心实体: " + ", ".join(h['entity'] for h in hubs[:4]))
    edges = (hi or {}).get("edges", []) if isinstance(hi, dict) else []
    teach_edges = [e for e in edges if isinstance(e, dict) and e.get("teaching_note")]
    if teach_edges:
        parts.append("可用于教学的案例启示: " + "; ".join(
            str(e.get("teaching_note", ""))[:60] for e in teach_edges[:3]
        ))
    return "\n".join(parts)
...
_COMPETITION_AGENT_HINTS: dict[str, dict[str, str]] = {
    "coach": {
        "internet_plus": "该学生在准备互联网+。互联网+评审重点关注商业模式闭环(价值主张→渠道→收入→成本)、TAM/SAM/SOM市场论证、路演叙事。教练时需围绕这些核心评审维度展开。",
        "challenge_cup": "该学生在准备挑战杯。挑战杯重点关注技术创新深度、实验验证、文献综述质量。教练时侧重创新可信度和科研规范。",
        "dachuang": "该学生在做大创项目。大创评审看重训练过程记录、方案可落地性、团队分工。教练时侧重可执行性和实践记录。",
    },
    "analyst": {
        "internet_plus": "互联网+评审角度：商业模式不完整、市场论证缺数据、路演逻辑断裂是最大风险。",
        "challenge_cup": "挑战杯评审角度：技术创新不足、实验设计不严谨、与现有方案对比缺失是最大风险。",
        "dachuang": "大创评审角度：方案不可执行、过程记录缺失、团队分工不清是最大风险。",
    },
    "advisor": {
        "internet_plus": "互联网+评审采用逐维度打分(商业模式权重20%、市场竞争15%、创新10%)，你的建议要帮学生在高权重维度不丢分。",
        "challenge_cup": "挑战杯评审创新权重最高(20%)，技术可行性和团队次之(各10-15%)。帮学生在创新维度拿高分。",
        "dachuang": "大创评审可行性权重最高(18%)，创新和过程记录次之。帮学生证明方案可落地。",
    },
    "grader": {
        "internet_plus": "按互联网+评分体系：商业模式一致性(20%)、市场与竞争(15%)、创新与路演(各10%)。缺失商业模式维度扣分最重。",
        "challenge_cup": "按挑战杯评分体系：创新与差异化(20%)、用户调研(15%)、技术可行性(15%)。缺失创新维度扣分最重。",
        "dachuang": "按大创评分体系：方案可行性(18%)、创新(15%)、团队与过程(12%)。可行性不足扣分最重。",
    },
    "planner": {
        "internet_plus": "互联网+备赛节奏：先补商业模式闭环证据，再做市场论证，最后打磨路演。任务要服务于这个优先序。",
        "challenge_cup": "挑战杯备赛节奏：先做实验/原型验证创新点，再完善技术文档，最后准备答辩。任务要服务于这个优先序。",
        "dachuang": "大创备赛节奏：先制定可执行计划和里程碑，边做边记录过程，最后整理成果报告。任务要服务于这个优先序。",
    },
    "tutor": {
        "internet_plus": "学生在准备互联网+，教学时多联系商业模式设计、市场验证方法、路演技巧等评审高频考点。",
        "challenge_cup": "学生在准备挑战杯，教学时多联系实验设计方法、创新性论证、技术文献综述写法等评审高频考点。",
        "dachuang": "学生在做大创项目，教学时多联系项目管理方法、可行性分析方法、过程性记录的写法。",
    },
}


def _get_competition_hint(agent_role: str, competition_type: str) -> str:
    """Return agent-specific hint for the given competition type, or empty string."""
    if not competition_type:
        return ""
    hint = _COMPETITION_AGENT_HINTS.get(agent_role, {}).get(competition_type, "")
    if hint:
        label = _COMPETITION_LABEL.get(competition_type, competition_type)
        return f"[赛事: {label}] {hint}"
    return ""
...
def _build_learning_tutor_reply(state: dict, structured: bool = True) -> tuple[str, list[dict[str, Any]]]:
    ...
    hi = state.get("hypergraph_insight", {})
    hyper_tutor_ctx = _fmt_hyper_for_tutor(hs, hi) if use_project_grounding else ""

    comp_type = state.get("competition_type", "") or ""
    tutor_comp_hint = _get_competition_hint("tutor", comp_type)
    _tutor_mode_hint = {
        "competition": "你现在还需要帮学生理解这个概念在竞赛评审中的权重和评委判断标准。",
        "learning": "你现在还需要帮学生理解这个概念在项目实操中如何验证，做完后该看什么信号判断自己学会了。",
    }.get(mode, "")
    if tutor_comp_hint:
        _tutor_mode_hint = f"{_tutor_mode_hint}\n{tutor_comp_hint}" if _tutor_mode_hint else tutor_comp_hint
...
def _coach_analyze(state: dict) -> dict:
    ...
    comp_type = state.get("competition_type", "") or ""
    comp_hint = _get_competition_hint("coach", comp_type)
...
def _analyst_analyze(state: dict) -> dict:
    ...
    hyper_analyst_ctx = _fmt_hyper_for_analyst(hs, hyper_insight)
    ...
    comp_type = state.get("competition_type", "") or ""
    comp_hint = _get_competition_hint("analyst", comp_type)
    _analyst_mode_hint = {
    ...
    if comp_hint:
        _analyst_mode_hint = f"{_analyst_mode_hint}\n{comp_hint}"
    ...
    return {
        "agent": "风险分析师",
        "analysis": analysis or "",
        "tools_used": ["diagnosis", "kg_analysis", "hypergraph", "neo4j", "web_search"] if ws_ctx else ["diagnosis", "kg_analysis", "hypergraph", "neo4j"],
    }
...
def _advisor_analyze(state: dict) -> dict:
    ...
    hyper_advisor_ctx = _fmt_hyper_for_advisor(hs, hyper_insight)
    ...
    comp_type = state.get("competition_type", "") or ""
    comp_hint = _get_competition_hint("advisor", comp_type)
    comp_urgency = "竞赛教练模式——学生正在备赛，请以专业评委和教练的标准帮助他提升获奖概率，但保持克制、严谨、基于证据。" if mode == "competition" else ""
    if comp_hint:
        comp_urgency = f"{comp_urgency}\n{comp_hint}" if comp_urgency else comp_hint
    ...
    return {
        "agent": "竞赛顾问",
        "analysis": analysis or "",
        "tools_used": ["competition_llm", "rubric_engine", "rag_reference", "hypergraph", "neo4j"],
...
def _grader_analyze(state: dict) -> dict:
    ...
    hs = state.get("hypergraph_student", {})
    hyper_grader_ctx = _fmt_hyper_for_grader(hs)

    neo4j_ctx = ""
    if _graph_service and kg.get("entities"):
        top_labels = [e["label"] for e in kg["entities"][:5] if e.get("label")]
        try:
            related = _graph_service.find_similar_entities(top_labels, limit=3)
            if related:
                neo4j_ctx = "图谱同类项目: " + "; ".join(
                    f"{r.get('entity','')}→{r.get('related_entity','')}"
    ...
    comp_type = state.get("competition_type", "") or ""
    comp_hint = _get_competition_hint("grader", comp_type)
...
def _planner_analyze(state: dict) -> dict:
    ...
    comp_type = state.get("competition_type", "") or ""
    comp_hint = _get_competition_hint("planner", comp_type)
    hyper_planner_ctx = _fmt_hyper_for_planner(hs)
    ...
    neo4j_ctx = ""
    if _graph_service and kg_entities:
        top_labels = [e["label"] for e in kg_entities[:5] if e.get("label")]
        try:
            related = _graph_service.find_similar_entities(top_labels, limit=4)
            if related:
                neo4j_ctx = "图谱相似项目参考: " + "; ".join(
                    f"{r.get('entity','')}→{r.get('related_entity','')}"
    ...
    if plan:
        ...
        if tasks:
            ...
            for t in tasks[:1]:
                ...
                parts.append(f"- **{t.get('task','')}**: {how_text}")
                if t.get("acceptance"):
...
def run_role_agents_node(state: WorkflowState) -> dict:
    ...
    agents_set = set(agents_to_run)

    # Dependency-aware phased parallel execution:
    #   Phase 1 (parallel): coach, tutor  (independent)
    #   Phase 2 (parallel): analyst, advisor, planner  (depend on coach_output)
    #   Phase 3:            grader  (depends on advisor_output)
    _P1 = {"coach", "tutor"}
    _P2 = {"analyst", "advisor", "planner"}
    _P3 = {"grader"}
    ...
    def _run_phase(keys: set[str], snapshot: dict) -> None:
        phase_agents = [k for k in agents_to_run if k in keys and k in agents_set]
        if not phase_agents:
            return
        if len(phase_agents) == 1:
            k, result = _run_one(phase_agents[0], snapshot)
            outputs[k] = result
            state_snapshot[f"{k}_output"] = result
        else:
            with ThreadPoolExecutor(max_workers=len(phase_agents)) as pool:
                futures = {pool.submit(_run_one, k, dict(snapshot)): k for k in phase_agents}
                for future in as_completed(futures):
                    k, result = future.result()
                    outputs[k] = result
                    state_snapshot[f"{k}_output"] = result

    _run_phase(_P1, state_snapshot)
    _run_phase(_P2, state_snapshot)
    _run_phase(_P3, state_snapshot)
...
_MODE_PERSONA: dict[str, str] = {
    "coursework": (
        ...
        "你同样精通创业实操知识：融资策略与估值方法、公司注册与股权架构、团队搭建与增长获客、现金流管理与财税合规。"
    ...
    "competition": (
        ...
        "你熟悉投资人视角、商业计划书评审标准、融资逻辑和团队组建策略。"
        "你的目标是帮助学生提高获奖概率，侧重评委视角、证据链、路演打磨和竞争力提升。"
        "语气专业、克制、严谨，像有依据的高水平教练。"
    ),
    "learning": (
        "你是一位真正负责推进项目的项目教练。"
        "你不仅懂创新方法论，还精通创业全流程实操：从注册公司到融资上市，从冷启动获客到团队管理，从股权架构到财税合规。"
...
_FOCUSED_PROMPTS: dict[str, str] = {
    ...
    "funding_investment": (
        "学生在问融资、估值、股权、投资人等创业融资相关问题。你的任务：\n"
        "1. 先判断学生处于什么阶段（想法期/种子期/成长期），融资建议必须匹配阶段\n"
        "2. 用通俗语言解释核心概念（如估值怎么定、股权怎么分、BP怎么写、Term Sheet 关键条款）\n"
        "3. 给出该阶段常见的融资路径和金额区间，说清楚天使轮/Pre-A/A轮各阶段投资人关注什么\n"
        "4. 如果有搜索结果，引用真实案例和数据（某公司融资历程、行业融资均值等）\n"
        "5. 指出学生最容易踩的坑（如过早稀释股权、估值虚高、对赌条款风险）\n"
        "6. 给出1-2个立刻可以做的准备动作\n"
        "7. 如果学生还没有产品或收入，坦然告知现阶段融资难度，建议先做什么再融资\n"
        "用700-1200字回复。专业但通俗，像一位有投资经验的学长在聊天。"
    ),
    "company_operations": (
        "学生在问注册公司、法务、股权架构、财税等公司运营问题。你的任务：\n"
        "1. 直接回答具体操作流程（如注册公司步骤、需要的材料、大约花费和时间）\n"
        "2. 解释不同公司类型的区别（有限责任公司 vs 合伙企业 vs 个体工商户），帮学生选择\n"
        "3. 如果涉及股权，讲清楚创始人之间怎么分股权、期权池要不要预留、代持风险\n"
        "4. 如果涉及知识产权，说明专利/商标/著作权的区别和申请流程\n"
        "5. 指出学生最容易忽略的法律风险（如合伙人没签协议、知识产权归属不清）\n"
        "6. 如果有搜索结果，引用最新政策或地区优惠（如大学生创业免税政策、孵化器补贴）\n"
        "7. 给出一个最该立刻做的事\n"
        "用700-1200字回复。像一个懂法律的创业前辈给建议，实操、具体、可执行。"
    ),
    "startup_execution": (
        "学生在问团队搭建、增长获客、运营、冷启动等创业执行问题。你的任务：\n"
        "1. 先判断学生项目阶段（0-1冷启动 / 1-10增长期 / 10-100规模化），执行建议必须匹配阶段\n"
        "2. 对于冷启动：讲清前100个种子用户怎么找、用什么渠道、怎么验证留存\n"
        "3. 对于增长：讲清获客渠道选择（私域/投放/内容/SEO/社群）、CAC控制、转化漏斗优化\n"
        "4. 对于团队：讲清创业早期招人策略（合伙人 vs 外包 vs 实习生）、股权激励时机\n"
        "5. 对于运营：讲清关键指标（DAU/MAU/留存/ARPU/LTV）怎么看、怎么优化\n"
        "6. 如果有搜索结果，引用真实公司的增长案例和数据\n"
        "7. 给出1-2个本周就能开始做的具体动作\n"
        "用700-1200字回复。像一个有实战经验的创业合伙人在讨论执行。"
    ),
    "general_chat": (
        "学生这条消息看起来不是在做深度项目分析。\n"
        "你的任务：\n"
...
def orchestrator(state: WorkflowState) -> dict:
    ...
    comp_type = state.get("competition_type", "") or ""
    comp_label = _COMPETITION_LABEL.get(comp_type, "")
    comp_orch_hint = f"\n[当前赛事: {comp_label}] 整合回复时，请确保建议符合{comp_label}的评审侧重点和备赛节奏。\n" if comp_label else ""
    ...
    if _llm.enabled and analyses_ctx:
        ...
        if is_file:
            length_guide = "2200-4200字。逐段分析文件内容，引用原文，对比案例，给出具体修改建议。"
        elif n_analyses >= 4:
            length_guide = "1800-3800字。充分整合所有分析维度，逐一覆盖每个重要发现和子问题。"
        elif n_analyses >= 3:
            length_guide = "1500-3200字。充分整合所有分析维度，每个重要发现都要展开讨论。"
        elif n_analyses >= 2:
            length_guide = "1200-2600字。深入分析关键问题，引用案例和证据，不遗漏重要发现。"
        elif msg_len > 200:
            length_guide = "900-1800字。针对学生具体内容深入展开。"
        else:
            length_guide = "600-1200字。聚焦问题给出有深度的建议。"
        ...
        reply = _llm.chat_text(
            system_prompt=(
                ...
                "- 如果分析中提出了4-6个有价值的质疑或发现，你应该尽量全部展开讨论\n"
                "- 如果学生在一条消息里问了多个子问题、多个场景或多个产品方向，要逐项回应，不要只答前三点\n"
                "- 每个发现都要结合学生的具体内容深入展开，给出数据、案例或逻辑推演\n"
                "- 如果诊断规则、KG结构缺口、超图风险和联网事实同时存在，要把这些线索串起来讲，不要只摘其中三条\n"
                "- 如果上下文里出现了联网搜索结果，请在合适位置引用来源名称，并在结尾整理1-3条可点击链接\n"
                "- 宁可回复长一点内容扎实，也不要为了简短而丢掉好的分析洞察\n\n"
                "- 如果这是基于上轮的继续追问，先用1-2句承接上轮判断，再只回答这轮新增问题，避免把上一轮整段分析重新讲一遍\n"
                "- 如果 continuation_mode 是 followup_scoring，你的主体任务是基于已有项目分析直接回答评分、扣分点和优先级，不要重新做一轮完整项目诊断\n"
                "- 如果 continuation_mode 是 followup_revision 或 followup_deepening，你应该默认学生认可上轮主判断，只继续把新增问题往下推\n"
                "- 除非已经明确提供了行动规划内容，否则不要把整篇回复写成任务分配单\n"
                "- 项目教练、风险分析、竞赛评估、课程辅导这些内容主要负责解释问题本身，为什么成立、卡在哪里、有哪些替代方案和外部事实\n"
                "- 整篇回复里，分析与解释至少占七成篇幅；行动与任务最多放在最后一小节收束\n"
                "- 如果已经有行动规划内容，只允许在最后单独一节引用它；前面各节禁止继续扩写成步骤、周计划或验收标准\n"
                "- 如果没有行动规划内容，最多只给1个简短的下一步聚焦点，不要写成本周计划\n\n"
                + (
                    "## 课程辅导模式额外要求\n"
                    "- 这轮回复的第一任务是把学生当前这一题讲明白，不要先急着做宏大项目诊断\n"
                    "- 如果课程导师已经提供了方法解释或判断标准，最终回复里必须保留这些教学性内容，不要只剩结论\n"
                    "- 语气要像老师面对面讲解：先说学生为什么会卡住，再解释老师通常怎么判断，再落回项目\n"
                    "- 即使项目教练或风险分析也参与了，最终成文仍然要像一节讲清楚问题的小辅导，而不是多份报告拼接\n\n"
                    if mode == "coursework"
                    else "## 竞赛教练模式额外要求\n"
                    ...
                    "- 分析应明显多于行动建议，启发式追问优先于直接给答案\n"
                    "- 如果学生还在方向探索期，带着他收敛场景和切口，而不是宣布'最大瓶颈'\n\n"
                )
                + "## 回复逻辑结构\n"
                "1. **总览**（1-2句）：概括你对项目的整体判断\n"
                "2. **逻辑链路**：用户是谁→痛点→方案→变现→壁垒，哪些通哪些断\n"
                "3. **结构层原因**：不只说表面问题，还要解释这些问题背后共同指向的结构性断点\n"
                "4. **策略空间**：说明学生有哪些可选打法、取舍和切口，不要把建议写成唯一答案\n"
                "5. **逐一展开核心问题**：将分析中发现的每个重要问题都展开讨论，"
                "引用学生原话，给具体例子或数据说明为什么这是问题、怎么改\n"
                f"6. **下一步行动**：{'仅根据已给出的行动规划内容，集中写在最后一节' if has_planner else '只给一个很短的下一步聚焦点，不展开成计划'}\n"
                "7. **追问**：按问题复杂度提出若干个有深度的苏格拉底式问题；不要固定三个，问题少就少给，问题多就多给\n"
                "8. **外部参考**：如果已提供联网结果，列出1-3个来源链接\n\n"
                "## 必须做到\n"
                "- 第一人称回复，像导师面对面聊天\n"
                "- 紧扣学生具体内容，引用原话讨论\n"
                "- 排版清晰：## 标题分块、> 引用突出、**加粗**关键词、表格呈现对比\n"
                "- 如果在比较竞品、路径、角色分工、收费方案，优先使用 Markdown 表格\n"
                "- 如果在解释流程、因果链、阶段推进、用户漏斗，必要时可输出 mermaid 代码块\n"
                "- 不要在末尾输出'⚠ AI生成，仅供参考'之类的免责声明，前端已经有了\n"
                f"- **回复长度**: {length_guide}\n"
            ),
            user_prompt=(
                f"学生说：\n{msg[:3000]}\n\n"
                ...
                + f"以下是问题分析素材（这些内容优先用于解释问题本身）：\n\n{analyses_ctx}\n\n"
                + (f"以下是行动规划素材（如果要写详细动作，只能在最后一节使用这一部分）：\n\n{planner_block}" if has_planner else "本轮没有独立行动规划素材；请把重点放在分析本身。")
            ),
        ...
        if intent == "general_chat":
            ...
            reply = _llm.chat_text(
                system_prompt=(
                    f"{persona}\n{focused_prompt}\n"
                    + (comp_orch_hint if comp_orch_hint else "")
                    + "\n## 通用要求\n"
                    "- 第一人称回复，像导师面对面聊天\n"
...
def stream_orchestrator(state: dict):
    ...
    system_prompt = (
        ...
        "- **严禁万金油建议**：不许动不动就建议'做用户访谈''发放问卷''做市场调研'。"
        "  如果确实需要调研，必须说清楚：调研什么假设、找什么人、问什么问题。\n\n"
        "- 如果分析素材里已经出现多条不同问题线索，不要默认只挑三条来讲，要尽量讲全主链路上的重要问题\n"
        "- 如果诊断规则、结构缺口、超图风险、联网事实同时存在，要把它们串起来讲，不要零散带过\n"
        "- 如果学生的判断过于直觉化、乐观化或忽略替代方案，要主动补入反直觉洞察：给具体企业案例、行业背景、公开数据或历史类比，而不是只给抽象提醒\n"
        "- 如果已有联网结果或案例知识，优先把这些事实嵌到分析里，并解释这个例子为什么对当前项目成立\n"
        "- 如果这是基于上轮的继续追问，先用1-2句承接上轮判断，再只回答这轮新增问题，避免把上一轮整段分析重新讲一遍\n"
        "- 如果 continuation_mode 是 followup_scoring，你的主体任务是基于已有项目分析直接回答评分、扣分点和优先级，不要重新做一轮完整项目诊断\n"
        "- 如果 continuation_mode 是 followup_revision 或 followup_deepening，你应该默认学生认可上轮主判断，只继续把新增问题往下推\n"
        "- 除非已经明确提供了行动规划内容，否则不要把整篇回复写成任务分配单\n"
        "- 整篇回复里，分析与解释至少占七成篇幅；行动与任务最多放在最后一小节收束\n"
        "- 如果已经有行动规划内容，只允许在最后单独一节引用；前面各节禁止继续扩写成步骤、周计划或验收标准\n"
        "- 如果没有行动规划内容，最多只给1个简短的下一步聚焦点，不展开成详细计划\n\n"
        + (
            "## 课程辅导模式额外要求\n"
            "- 这轮回复的第一任务是把学生当前这一题讲明白\n"
            "- 保留课程导师的方法解释和判断标准，不要只剩结论\n"
            "- 语气像老师面对面讲解\n\n"
            if mode == "coursework"
            else "## 竞赛教练模式额外要求\n"
        ...
        "3. 结构层原因（解释这些表层问题背后的共同断点）\n"
        "4. 反直觉提醒/外部参照（如果适用，补1-3个容易被忽略的案例、数据或行业事实）\n"
        "5. 策略空间（说明可选打法、取舍与切口，不要只有唯一答案）\n"
        "6. 逐项展开所有重要问题或子问题，不要只选前两点\n"
        f"7. {'集中收束行动规划（仅当已提供行动规划素材）' if has_planner else '最后只点出一个最该先盯住的点'}\n"
        "8. 引导性追问（按复杂度给若干个，不要固定三个）\n"
        "9. 如果有联网资料，补充来源链接\n\n"
        "## 必须做到\n"
        "- 以第一人称'我'回复\n"
        "- 紧扣学生具体内容，引用原话\n"
        "- 排版美观：## ### 标题、表格、> 引用块、**加粗**\n"
        "- 适合比较时优先使用 Markdown 表格；适合流程、因果链、阶段推进时可输出 mermaid 代码块\n"
        f"- 回复长度: {length_guide}\n"
    )
