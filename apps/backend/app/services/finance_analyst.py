...
def analyze_unit_economics(
    ...
    *,
    ...
    if price <= 0 or cac <= 0 or retention <= 0:
        ...
        card["framework_explain"] = (
            "单位经济（Unit Economics）衡量「每获得一个用户在其生命周期内能赚多少」。"
            "核心公式：LTV = 月 ARPU × 毛利率 × 平均生命周期月数；LTV/CAC ≥ 3 视为健康。"
        )
        return card
    ...
    if ratio >= 3 and math.isfinite(payback) and payback <= 12:
        ...
        card["evidence_for_diagnosis"]["H8"] = 0.2
    return card
...
def _build_ue_suggestions(ratio: float, payback: float, price: float, baseline: dict, ind: str) -> list[str]:
    ...
    elif price_lo > 0 and price < price_lo * 0.7:
        out.append(f"当前月价 ¥{price:.0f} 低于{ind}行业下限 ¥{price_lo}，警惕低价陷阱：高留存难以覆盖低毛利")
    return out
...
def project_cash_flow(assumptions: dict, months: int = 36) -> dict[str, Any]:
    ...
    if fixed <= 0 and price <= 0:
        card["verdict"]["reason"] = "缺少月固定成本或定价，无法模拟现金流"
        card["framework_explain"] = (
...
def evaluate_rationality(
    ...
    *,
    ...
    """
    对 collected 里所有能和行业基准对比的字段做红/黄/绿打分。
    不强制要求所有字段齐全——用多少字段评多少。
    allow_online: 是否允许基线数据联网刷新。
    """
    ind = _match_industry(industry)
    baseline = _get_baseline(industry, allow_online=allow_online)
    ...
    def _check(field: str, range_key: str, label: str, risk_side: str = "both") -> None:
        """
        risk_side:
          - "both"      → 过高或过低都当作风险（默认，用于 monthly_price / retention / margin）
          - "only_high" → 只有过高才是风险；过低反而是优势（用于 CAC）
          - "only_low"  → 只有过低才是风险；过高更好（暂未使用）
        """
        v = _safe_float(collected.get(field))
        if v <= 0:
            return
        ...
        if _in_range(v, lo, hi):
            checks.append({"field": field, "value": v, "range": [lo, hi], "level": "green",
                           "note": f"{label} {v} 落在行业区间 [{lo}, {hi}]"})
            levels.append("green")
            return
        ...
        # 根据 risk_side 决定偏离是否算风险
        if risk_side == "only_high" and not is_above:
            # CAC 低于区间：竞争优势，不算风险
            checks.append({
                "field": field, "value": v, "range": [lo, hi], "level": "green",
                "note": f"{label} {v} 低于行业区间 [{lo}, {hi}]——若真实可达则是竞争优势，建议用渠道成本核算复核一次",
            })
            levels.append("green")
            return
        if risk_side == "only_low" and is_above:
            checks.append({
                "field": field, "value": v, "range": [lo, hi], "level": "green",
                "note": f"{label} {v} 高于行业区间 [{lo}, {hi}]——超过基线，可作为亮点",
            })
            levels.append("green")
            return

        # 确有风险：按偏离幅度给红/黄
        if (v < lo * 0.6) or (v > hi * 1.5):
            level = "red"
            direction = "远低于" if v < lo else "远高于"
        else:
            level = "yellow"
            direction = "略低于" if v < lo else "略高于"
        checks.append({"field": field, "value": v, "range": [lo, hi], "level": level,
                       "note": f"{label} {v} {direction}行业区间 [{lo}, {hi}]"})
        levels.append(level)

    _check("monthly_price", "monthly_price_range", "月价", risk_side="both")
    _check("cac", "cac_range", "CAC", risk_side="only_high")
    _check("monthly_retention", "monthly_retention", "月留存率", risk_side="both")
    _check("gross_margin", "gross_margin", "毛利率", risk_side="both")
    ...
    card["outputs"] = {
        ...
        "baseline": {
            k: baseline.get(k)
            for k in ("cac_range", "monthly_price_range", "monthly_retention", "gross_margin")
        },
    }

    if not checks:
        card["verdict"]["reason"] = "没有任何可与行业基准对比的数字，先补 CAC/留存/定价/毛利 中的至少一项"
        card["missing_inputs"] = [
    ...
    worst = max(levels, key=lambda l: order.get(l, 0))
    red_count = sum(1 for l in levels if l == "red")
    yellow_count = sum(1 for l in levels if l == "yellow")
    reason_parts = []
    for c in checks:
        if c["level"] != "green":
    ...
    card["verdict"] = {
        "level": worst,
        "score": {"green": 0.9, "yellow": 0.55, "red": 0.25}[worst],
        "reason": "；".join(reason_parts) if reason_parts else f"{len(checks)} 项假设均落在行业区间内",
    }

    card["framework_explain"] = (
        f"**行业基准对比**（{ind}）：用头部项目的中位区间做锚，判断你的假设是否在同行合理范围。\n"
        f"- 月价 [{baseline.get('monthly_price_range', '—')}]\n"
        f"- CAC [{baseline.get('cac_range', '—')}]\n"
        f"- 月留存 [{baseline.get('monthly_retention', '—')}]\n"
        f"- 毛利率 [{baseline.get('gross_margin', '—')}]\n\n"
        f"{baseline.get('note', '')}"
    )
    ...
    for c in checks:
        if c["level"] == "red":
            sugg.append(f"{c['note']}——需要用用户调研或成本拆解补充证据")
        elif c["level"] == "yellow":
            sugg.append(f"{c['note']}——可通过 A/B 测试或竞品抽样进一步验证")
    if not sugg:
        sugg.append("财务假设整体合理，建议进入下一步：情景分析 + 敏感性扫描")
    card["suggestions"] = sugg

    # 证据回流
    if red_count == 0 and yellow_count <= 1:
        card["evidence_for_diagnosis"]["H3"] = 0.8
        card["evidence_for_diagnosis"]["H26"] = 0.7
    elif red_count >= 2:
        card["evidence_for_diagnosis"]["H3"] = 0.25
    else:
        card["evidence_for_diagnosis"]["H3"] = 0.5
...
def estimate_market_size(
    ...
    """
    输入仅为项目描述 + 可选 hints（比如 target_user_population / channel_reach / conversion）。
    做自上而下 + 自下而上双路估算并交叉比对。
    """
    ind = _match_industry(industry)
    ...
    if pop > 0 and arpu_yr > 0:
        # 假设你能服务的是 TAM 的 1/3（同行平均）
        outputs["bottom_up_SAM_est"] = round(pop * arpu_yr / 3, 2)
    ...
    # 交叉比对 + verdict
    gap_note = ""
    if "bottom_up_TAM" in outputs and "top_down_TAM" in outputs:
        bu = outputs["bottom_up_TAM"]
        td = outputs["top_down_TAM"]
        if td > 0:
            ratio = bu / td
            if 0.3 <= ratio <= 3.0:
                gap_note = f"自下而上 ¥{bu:.0f} vs 自上而下 ¥{td:.0f}（比值 {ratio:.2f}）—— 交叉验证合理"
                card["verdict"] = {"level": "green", "score": 0.85, "reason": gap_note}
                card["evidence_for_diagnosis"]["H4"] = 0.75
                card["evidence_for_diagnosis"]["H19"] = 0.7
            else:
                gap_note = f"自下而上 ¥{bu:.0f} vs 自上而下 ¥{td:.0f}（比值 {ratio:.2f}）—— 两种方法差距 {ratio:.1f}x，需排查口径"
                card["verdict"] = {"level": "yellow", "score": 0.5, "reason": gap_note}
                card["evidence_for_diagnosis"]["H4"] = 0.4
    elif "bottom_up_SOM_yr1" in outputs:
        card["verdict"] = {
            "level": "yellow",
            "score": 0.6,
            "reason": f"只有自下而上估算（SOM 首年 ¥{outputs['bottom_up_SOM_yr1']:.0f}），建议补一个自上而下数字做交叉",
        }
        card["evidence_for_diagnosis"]["H19"] = 0.55
    else:
        card["verdict"]["reason"] = "缺少关键人口/转化/单价数据，无法建模 TAM/SAM/SOM"

    # 简单 sanity：懒惰估法识别
    lazy_hint = "1%" in description or "百分之一" in description or "拿到" in description
    if lazy_hint:
        card["suggestions"].append("检测到「拿到 1% 市场」式懒惰估算，建议改自下而上：渠道×转化×单价")
    ...
    card["framework_explain"] = (
        ...
        "**必须做交叉验证**：自上而下（行业报告市场规模 × 你的份额假设）+ 自下而上（可触达人数 × 转化率 × 客单价），两者偏差 < 3 倍才可信。"
    )
    card["suggestions"] += [
        "明确「首年可触达用户」对应哪些具体渠道（至少列 2 个）",
        "把 TAM/SAM/SOM 三个数字用漏斗图而不是堆叠饼图画出来",
    ]
...
_FLAT_BUDGET_KEYS = (
    ...
    "paid_conversion_rate", "target_user_population", "annual_arpu",
)


def extract_assumptions_from_budget(budget: dict | None) -> dict[str, Any]:
    """把 data/budgets/{user}/{plan}.json 里的 business_finance 拍成 flat assumptions。
    ...
    price = _safe_float(stream0.get("price"))
    conv = _safe_float(stream0.get("conversion_rate"))
    users = _safe_float(stream0.get("monthly_users"))
    out: dict[str, Any] = {}
    if price > 0:
